@echo off
REM Kong Gateway Service Registration Script - Local Development
REM This script registers all VNO services to Kong Gateway running on localhost
REM Usage: register-services-local.bat
...
echo.
echo Registering VNO Services to Kong Gateway (Local Development)
...
curl -s -X PUT %KONG_ADMIN_URL%/services/auth-service -d "name=auth-service" -d "url=http://host.docker.internal:8080" >nul
if errorlevel 1 (
...
curl -s -X PUT %KONG_ADMIN_URL%/services/user-service -d "name=user-service" -d "url=http://host.docker.internal:8081" >nul
if errorlevel 1 (
...
curl -s -X PUT %KONG_ADMIN_URL%/services/note-service -d "name=note-service" -d "url=http://host.docker.internal:8082" >nul
if errorlevel 1 (
...
curl -s -X PUT %KONG_ADMIN_URL%/services/realtime-collab-service -d "name=realtime-collab-service" -d "url=http://host.docker.internal:8083" >nul
if errorlevel 1 (
...
curl -s -X PUT %KONG_ADMIN_URL%/services/notification-producer -d "name=notification-producer" -d "url=http://host.docker.internal:8084" >nul
if errorlevel 1 (
...
curl -s -X PUT %KONG_ADMIN_URL%/services/notification-processor -d "name=notification-processor" -d "url=http://host.docker.internal:8085" >nul
if errorlevel 1 (
